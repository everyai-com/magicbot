    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      const launchAttempt = async (attempt: number): Promise<void> => {
        ...
        if (turn.integrations?.composio) {
          mountMcpServer(appServerArgs, env, "magicbot_connectors", turn.integrations.composio);
        }
        ...
        if (turn.integrations?.phone) {
          ...
          const prefix = "mcp_servers.magicbot_phone";
          appServerArgs.push(
      ...
      const settle = (ok: boolean, stopReason: string | null) => {
        ...
        state.settled = true;
        for (const finish of [...asks.values()]) finish("deny", "MagicBot: the turn ended", "system");
      ...
      try {
        await request("initialize", { clientInfo: { name: "magicbot", version: "1" } });
