    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      const launchAttempt = async (attempt: number): Promise<void> => {
        ...
        if (turn.integrations?.composio) {
          mountMcpServer(appServerArgs, env, "magicbots_connectors", turn.integrations.composio);
        }
        ...
        if (turn.integrations?.computer) {
          ...
          mountMcpServer(appServerArgs, env, "computer", {
            ...
            env: {
              ...
              MB_CONTROL_URL: proxyEnv.MB_CONTROL_URL ?? "",
              MB_CONTROL_TOKEN: proxyEnv.MB_CONTROL_TOKEN ?? "",
            },
          });
        ...
        if (turn.integrations?.phone) {
          ...
          const prefix = "mcp_servers.magicbots_phone";
          appServerArgs.push(
      ...
      const settle = (ok: boolean, stopReason: string | null) => {
        ...
        state.settled = true;
        for (const finish of [...asks.values()]) finish("deny", "MagicBots: the turn ended", "system");
      ...
      try {
        await request("initialize", { clientInfo: { name: "magicbots", version: "1" } });
