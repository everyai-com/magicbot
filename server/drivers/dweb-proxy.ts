...
// Exposes dweb's HTTP API (the local MagicBots network daemon) as tools an
// agent can call to inspect the network and run model requests:
//
//   dweb_status          → ping dweb, summarize server + peer state
//   dweb_repo_status     → the MagicBots repo the daemon is tracking
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Report the state of the MagicBots repository the dweb daemon is tracking: repo name, checked-out branch, last commit, file count, and checkout path.",
    inputSchema: { type: "object", properties: {} },
  },
  {
