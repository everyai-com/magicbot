...
export function resolveAdbPath(env: NodeJS.ProcessEnv = process.env, platform = process.platform): string | null {
  ...
  const candidates = [
    env.MB_ADB_PATH,
    env.MB_RESOURCES_PATH && join(env.MB_RESOURCES_PATH, "android-platform-tools", platform, executable),
...
async function runAdb(args: string[], options: { binary?: boolean; timeoutMs?: number } = {}): Promise<Buffer> {
  ...
  if (!adb) throw new Error("Android platform tools are unavailable. Reopen MagicBots or install adb.");
...
async function readNodes(serial: string) {
  ...
  const remotePath = "/data/local/tmp/magicbots-window.xml";
...
async function handle(message: Json) {
  ...
  if (method === "initialize") return ok(id, { protocolVersion: String(params.protocolVersion ?? "2024-11-05"), capabilities: { tools: {} }, serverInfo: { name: "magicbots-phone", version: "1" } });
