...
// there as first-class pi tools (pi.registerTool). MagicBot ships this file
// and the pi driver spawns it — the pi repo itself is never touched.
//
// Protocol: the MagicBot proxies speak raw JSON-RPC 2.0 over stdio, one
...
/** A minimal stdio JSON-RPC 2.0 MCP client, matched to MagicBot's
...
class StdioMcp {
  ...
  async init(): Promise<void> {
    await this.call("initialize", {
      ...
      capabilities: {},
      clientInfo: { name: "magicbot-pi", version: "1" },
    });
...
export default async function (pi: ExtensionAPI): Promise<void> {
  ...
  for (const [serverName, def] of Object.entries(config.mcpServers ?? {})) {
    ...
    try {
      ...
      process.stderr.write(`[magicbot-pi-mcp] ${serverName}: ${message}\n`);
      client?.dispose();
    }
  }
