...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `MAGICBOTS_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.MAGICBOTS_PROBE_LOCAL_INJECT ?? process.env.MAGICBOTS_PROBE_LOCAL_INJECT;
