...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `MAGICBOT_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.MAGICBOT_PROBE_LOCAL_INJECT ?? process.env.MAGICBOT_PROBE_LOCAL_INJECT;
