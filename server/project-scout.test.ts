...
function project(files: Record<string, string>): string {
  const dir = mkdtempSync(join(tmpdir(), "mb-scout-"));
  dirs.push(dir);
...
describe("scoutProject", () => {
  it("names the project from the README and summarizes its first paragraph", () => {
    const dir = project({
      "README.md": "[![ci](x)](y)\n# Bot Tracker\n\nTracks every bot in the house.\n\nMore prose.",
      "package.json": JSON.stringify({ name: "not-this", description: "not this either" }),
    });
    const profile = scoutProject(dir);
    expect(profile.name).toBe("Bot Tracker");
    expect(profile.summary).toBe("Tracks every bot in the house.");
  });
...
describe("suggestTeam", () => {
  const profile: ProjectProfile = {
    name: "Bot Tracker",
    summary: "Tracks every bot in the house.",
  ...
  it("always leads with a lead, then one member per signal, each with a reason", () => {
    ...
    expect(suggestion.roomName).toBe("Bot Tracker");
    ...
    expect(frontend.description).toContain("Bot Tracker");
