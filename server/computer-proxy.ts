...
const CONTROL_POLL_MS = Math.max(Number(process.env.MB_CONTROL_POLL_MS) || 1_500, 25);
const CONTROL_WAIT_MS = Math.max(Number(process.env.MB_CONTROL_WAIT_MS) || 600_000, 100);
...
const CHROME_PROFILE = "$HOME/.magicbots/chrome-profile";
...
const CHROME_PROFILE_SETUP = [
  ...
  "      exit 1",
  "    fi",
  '    mv "$browser_dir" "$browser_dir.pre-magicbots-$(date +%s)-$$"',
  "  fi",
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "magicbots-computer", version: "3" },
      },
    });
  }
