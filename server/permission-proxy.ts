...
// MagicBots server and waits for the human's answer.
//
...
const dead = () => {
  for (const resolve of waiting.values()) {
    resolve({ behavior: "deny", message: "MagicBots: permission broker unavailable — skip this action" });
  }
  waiting.clear();
};
conn.on("error", dead);
...
const TOOLS = [
  {
    name: "approve",
    description: "Ask the MagicBots user whether a tool use is allowed",
    inputSchema: {
      type: "object",
      properties: {
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "magicbots-permissions", version: "1" },
      },
    });
  }
  ...
  if (msg.method === "tools/call") {
    ...
    if (!isQuestion) {
      if (answer.behavior === "allow") {
        ...
        text = JSON.stringify({ behavior: "deny", message: answer.message || "Denied from MagicBots" });
      }
    }
