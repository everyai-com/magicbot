// Config + data dirs. One file, ~/.magicbot/config.json, env fallbacks:
...
const appConfigSchema = z.object({
  ...
  /** MagicBot cloud computer running on the user's own Cloudflare account
   * (the hosted Box replacement — see cf-computer/). url = the deployed
   * Worker origin, token = the MAGICBOT_COMPUTER_TOKEN secret set on it. */
...
export const DATA_DIR = process.env.OMB_DATA_DIR ?? join(homedir(), ".magicbot");
...
export function loadConfig(): AppConfig {
  ...
  if (process.env.MAGICBOT_COMPUTER_URL !== undefined) cfg.cfComputer.url = process.env.MAGICBOT_COMPUTER_URL;
  if (process.env.MAGICBOT_COMPUTER_TOKEN !== undefined) cfg.cfComputer.token = process.env.MAGICBOT_COMPUTER_TOKEN;
