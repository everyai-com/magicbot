...
export const VPS_MANAGED_LABEL = "com.magicbot.vps";
export const VPS_CONTAINER_LABEL = "com.magicbot.container";
export const VPS_CONTAINER_PREFIX = "magicbot-vps";
...
const SCREENSHOT_PATH = "/tmp/magicbot-vps-preview.png";
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned MagicBot Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No MagicBot container exists for this bot on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted MagicBot image";
  if (!status.managed) return "The VPS container name is occupied by a container MagicBot did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing MagicBot safety limits";
  if (status.container === "stopped") return "The MagicBot VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        // never docker-rm a container MagicBot did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container MagicBot did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
