...
export const VPS_MANAGED_LABEL = "com.magicbots.vps";
export const VPS_CONTAINER_LABEL = "com.magicbots.container";
export const VPS_CONTAINER_PREFIX = "magicbots-vps";
...
const SCREENSHOT_PATH = "/tmp/magicbots-vps-preview.png";
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned MagicBots Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No MagicBots container exists for this bot on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted MagicBots image";
  if (!status.managed) return "The VPS container name is occupied by a container MagicBots did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing MagicBots safety limits";
  if (status.container === "stopped") return "The MagicBots VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        // never docker-rm a container MagicBots did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container MagicBots did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
