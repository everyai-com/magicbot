...
// into first-class MagicBot chat cards. The agent never authors an auth
...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (slugs.length) {
      ...
      send(textResult(
        id,
        `MagicBot showed the user a secure connection card for ${slugs.join(", ")}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
    if (/WAIT_FOR_CONNECTIONS$/i.test(name)) {
      send(textResult(id, "MagicBot is handling connection completion and will continue the task automatically."));
      return;
    }
  }
