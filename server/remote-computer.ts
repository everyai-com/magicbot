...
// inside the VM and MagicBot never exposes another inbound port.
...
export const REMOTE_CUA_SESSION = "magicbot";
export const REMOTE_CDP_HELPER = "/opt/ogb/magicbot-cdp.mjs";
...
export function remoteComputerBootstrapCommand(botName: string): string {
  ...
  return [
    ...
    `tmux has-session -t work 2>/dev/null || tmux new-session -d -s work 'echo; echo "  ▦ ${safeName}'"'"'s computer — MagicBot"; echo; exec bash -i'`,
    "echo bootstrapped",
  ].join("\n");
}
