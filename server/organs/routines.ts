...
export interface Routine {
  id: string;
  botId: string;
  /** the prompt run each time, e.g. "summarise my unread email" */
  prompt: string;
  /** interval in minutes: 60 hourly, 1440 daily, etc. Min 1. */
  everyMinutes: number;
  enabled: boolean;
  createdAt: number;
  lastRunAt?: number;
...
function saveAll(list: Routine[]): void {
  mkdirSync(DATA_DIR, { recursive: true });
  writeFileSync(ROUTINES_FILE, JSON.stringify(list, null, 2));
}

export function forBot(botId: string): Routine[] {
  return loadAll().filter((r) => r.botId === botId);
}

export function get(id: string): Routine | null {
  return loadAll().find((r) => r.id === id) ?? null;
}

export function create(botId: string, prompt: string, everyMinutes: number): Routine {
...
export function patch(id: string, p: Partial<Pick<Routine, "prompt" | "everyMinutes" | "enabled">>): Routine | null {
  const list = loadAll();
  const routine = list.find((r) => r.id === id);
  if (!routine) return null;
  if (p.prompt !== undefined) routine.prompt = p.prompt.trim();
  if (p.everyMinutes !== undefined) routine.everyMinutes = Math.max(1, Math.floor(p.everyMinutes) || 1);
  if (p.enabled !== undefined) routine.enabled = p.enabled;
...
/** A routine runner: fire the prompt as a turn on the bot. Returns false when
 *  the bot couldn't run it now (busy / unavailable) so we retry next tick. */
export type RoutineRunner = (botId: string, prompt: string) => Promise<void>;
...
export async function runDue(runner: RoutineRunner, now = Date.now()): Promise<void> {
  const list = loadAll();
  let changed = false;
  for (const routine of list) {
    if (!routine.enabled || routine.nextRunAt > now) continue;
    try {
...
export function startScheduler(runner: RoutineRunner): void {
  if (timer) return;
  timer = setInterval(() => {
    void runDue(runner);
  }, 30_000);
