...
//
// MagicBots owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/magicbots/cua-local-vm";
export const IMAGE_LAYER_VERSION = "4";
export const IMAGE_LAYER_LABEL = "com.magicbots.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "magicbots-computer";
export const MANAGED_LABEL = "com.magicbots.local-vm";
export const DRIVER_LABEL = "com.magicbots.cua-driver";
export const BASE_IMAGE_LABEL = "com.magicbots.cua-base";
export const WORKSPACE_LABEL = "com.magicbots.workspace";
export const TARGET_LABEL = "com.magicbots.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/magicbots-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/magicbots/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-magicbots-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-magicbots-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-magicbots-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-magicbots-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-magicbots-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:magicbots-cua-driver]' \\
      'command=/usr/local/bin/start-magicbots-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by MagicBots; recreate it";
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared MagicBots derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/magicbots-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "magicbots-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/magicbots-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
export function containerComputerMcp(
  ...
  return {
    ...
    env: {
      ...
      ...(control ? { MB_CONTROL_URL: control.url, MB_CONTROL_TOKEN: control.token } : {}),
    },
  };
}
...
/** Cloud boxes still use MagicBots's high-latency REST adapter. Local VMs
...
export function computerProxyEnv(
  ...
  return {
    ...
    ...(computer.control
      ? { MB_CONTROL_URL: computer.control.url, MB_CONTROL_TOKEN: computer.control.token }
      : {}),
  };
}
