...
//
// MagicBot owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/magicbot/cua-local-vm";
export const IMAGE_LAYER_VERSION = "4";
export const IMAGE_LAYER_LABEL = "com.magicbot.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "magicbot-computer";
export const MANAGED_LABEL = "com.magicbot.local-vm";
export const DRIVER_LABEL = "com.magicbot.cua-driver";
export const BASE_IMAGE_LABEL = "com.magicbot.cua-base";
export const WORKSPACE_LABEL = "com.magicbot.workspace";
export const TARGET_LABEL = "com.magicbot.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/magicbot-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/magicbot/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-magicbot-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-magicbot-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-magicbot-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-magicbot-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-magicbot-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:magicbot-cua-driver]' \\
      'command=/usr/local/bin/start-magicbot-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by MagicBot; recreate it";
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared MagicBot derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/magicbot-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "magicbot-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/magicbot-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
/** Cloud boxes still use MagicBot's high-latency REST adapter. Local VMs
