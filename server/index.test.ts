...
beforeAll(async () => {
  ...
  mkdirSync(join(home, ".magicbot"), { recursive: true });
  mkdirSync(join(staticDir, "assets"), { recursive: true });
  writeFileSync(join(staticDir, "index.html"), "<!doctype html><title>Packaged MagicBot</title>");
  writeFileSync(join(staticDir, "assets", "smoke.css"), "body { color: white; }");
  writeFileSync(
    join(home, ".magicbot", "config.json"),
    JSON.stringify({
      instances: {
  ...
  writeFileSync(
    join(home, ".magicbot", "groups.json"),
    JSON.stringify([
      {
        id: "test-dm",
  ...
  writeFileSync(
    join(home, ".magicbot", "messages-test-stranded-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".magicbot", "messages-test-cancel-room-thread.json"),
    JSON.stringify({
...
describe("harness HTTP API", () => {
  ...
  it("serves packaged UI assets and preserves API 404s", async () => {
    ...
    expect(await root.text()).toContain("Packaged MagicBot");
    ...
    expect(await spa.text()).toContain("Packaged MagicBot");
  ...
  it("exports every visible bot and imports the team without creating a room", async () => {
    ...
    expect((await api("POST", "/api/teams/export", {})).body.team.name).toBe("My MagicBot Team");
  ...
  it("validates and persists the global room turn timeout", async () => {
    ...
    const disk = JSON.parse(readFileSync(join(home, ".magicbot", "config.json"), "utf8"));
  ...
  it.skipIf(process.platform === "win32")("stores the credentials file with owner-only permissions", () => {
    expect(statSync(join(home, ".magicbot", "config.json")).mode & 0o777).toBe(0o600);
  });
  ...
  it("creates an independent webhook, accepts a delivery, deduplicates it, and rotates its secret", async () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(home, ".magicbot", "webhooks.json")).mode & 0o777).toBe(0o600);
    }
  });
...
describe("bot memory API", () => {
  ...
  const workspaceOf = (botId: string) => join(home, ".magicbot", "workspaces", botId);
  ...
  it("refuses every coat of path traversal without reading the target", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".magicbot", "secret.md"), "TOP-SECRET-MARKER sibling");
