...
describe("computer proxy (fake box)", () => {
      ...
      if (url.pathname.endsWith("/commands")) {
        ...
        req.on("end", () => {
          ...
          const stdout = command.includes("127.0.0.1:9222/json/list")
            ...
            : command.includes("magicbot-cdp.mjs snapshot")
              ? JSON.stringify({
                  title: "Account",
              ...
              : command.includes("magicbot-cdp.mjs click") || command.includes("magicbot-cdp.mjs fill")
                ? `GEOM 1920 1080\nHASH ${hash}\nSIZE ${size}\nB64 ${JPEG}\nSEM ok\n`
  ...
  it("uses fresh semantic browser refs without exposing URL secrets", async () => {
    ...
    expect(commands.length - before).toBe(1);
    expect(commands.at(-1)).toContain("magicbot-cdp.mjs fill");
  ...
  it("uses a private Chrome profile, strips URL credentials, and reports redirects", async () => {
    ...
    expect(issued[0]).toContain('profile="$HOME/.magicbot/chrome-profile"');
    ...
    expect(issued[0]).toContain('--user-data-dir="$HOME/.magicbot/chrome-profile"');
