...
const TOOLS = [
    {
        name: "computer_exec",
        description: "Run a shell command on the bot's cloud computer (a persistent Linux sandbox — its disk survives between turns). Returns stdout/stderr/exit code.",
        inputSchema: { type: "object", properties: { command: { type: "string" } }, required: ["command"] },
    },
    {
        name: "run_code",
        description: "Execute a code snippet on the cloud computer via its interpreter and get the results (rich outputs, no shell quoting hassles).",
        inputSchema: {
            type: "object",
            properties: {
                code: { type: "string" },
    ...
    {
        name: "expose_port",
        description: "Expose a port the cloud computer is listening on as a public preview URL — start the server first (e.g. with computer_exec, backgrounded), then expose it.",
...
async function toolCall(id, name, args) {
    if (name === "computer_exec") {
        ...
        if (!command)
            return text(id, "computer_exec needs a command", true);
        const out = await call("exec", { command });
        return text(id, `exit ${out.exitCode}\n${String(out.stdout ?? "").slice(-6000)}${out.stderr ? `\n[stderr]\n${String(out.stderr).slice(-2000)}` : ""}`, out.exitCode !== 0);
    }
    if (name === "run_code") {
        const code = String(args.code ?? "");
        if (!code)
            return text(id, "run_code needs code", true);
        const out = await call("run", { code, language: args.language || "python" });
        const parts = [];
        const logs = out.logs ?? {};
        if (Array.isArray(logs.stdout) && logs.stdout.length)
            parts.push(logs.stdout.join("\n"));
        if (Array.isArray(logs.stderr) && logs.stderr.length)
            parts.push(`[stderr]\n${logs.stderr.join("\n")}`);
        if (Array.isArray(out.results) && out.results.length)
            parts.push(JSON.stringify(out.results, null, 2));
        if (out.error)
    ...
    if (name === "write_file") {
        ...
        if (!path || typeof args.content !== "string")
            return text(id, "write_file needs path + content", true);
    ...
    if (name === "read_file") {
        const path = String(args.path ?? "");
        if (!path)
            return text(id, "read_file needs a path", true);
    ...
    if (name === "expose_port") {
        ...
        if (!Number.isFinite(port) || port <= 0)
            return text(id, "expose_port needs a numeric port", true);
...
async function handle(msg) {
    ...
    if (msg.method === "tools/call") {
        try {
            ...
            return text(msg.id, `cloud computer tool failed: ${e.message}`, true);
        }
    }
