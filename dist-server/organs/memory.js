...
const memoryFile = (botId) => join(DATA_DIR, `memory-${botId}.json`);
const MAX_FACTS = 50;
function load(botId) {
    try {
...
export function remember(botId, text) {
    const clean = text.trim();
    if (!clean)
        return null;
    const facts = load(botId);
    if (facts.some((f) => f.text === clean))
        return null;
    const fact = {
...
export function forget(botId, factId) {
    save(botId, load(botId).filter((f) => f.id !== factId));
}
...
export function captureFromText(botId, text) {
    const captured = [];
    let m;
