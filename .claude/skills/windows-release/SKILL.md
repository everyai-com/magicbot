...
description: Build, verify, and publish the Windows desktop build (NSIS installer + latest.yml) to the magicbot-releases repo. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `MagicBots-<version>-setup.exe` and its update feed to
[everyai-com/magicbot-releases](https://github.com/everyai-com/magicbot-releases).
...
| File | Purpose |
|---|---|
| `MagicBots-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `MagicBots-<version>-setup.exe.blockmap` | differential updates |
| `MagicBots-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `everyai-com/magicbot-releases` and, while the
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\MagicBots\logs\server.log`.
...
```powershell
Copy-Item release/MagicBots-<version>-setup.exe release/MagicBots-setup.exe
gh release upload v<version> --repo everyai-com/magicbot-releases `
  release/MagicBots-<version>-setup.exe `
  release/MagicBots-setup.exe `
  release/MagicBots-<version>-setup.exe.blockmap `
  release/latest.yml
```

Both names are required, for different consumers:

- **`MagicBots-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`MagicBots-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/MagicBots-setup.exe` button a stable URL. This
  mirrors `MagicBots.dmg` sitting beside `MagicBots-<version>.dmg`.
