...
description: Build, verify, and publish the Windows desktop build (NSIS installer + latest.yml) to the magicbot repo. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `MagicBot-<version>-setup.exe` and its update feed to
[everyai-com/magicbot](https://github.com/everyai-com/magicbot).
...
| File | Purpose |
|---|---|
| `MagicBot-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `MagicBot-<version>-setup.exe.blockmap` | differential updates |
| `MagicBot-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `everyai-com/magicbot` and, while the
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\MagicBot\logs\server.log`.
...
```powershell
Copy-Item release/MagicBot-<version>-setup.exe release/MagicBot-setup.exe
gh release upload v<version> --repo everyai-com/magicbot `
  release/MagicBot-<version>-setup.exe `
  release/MagicBot-setup.exe `
  release/MagicBot-<version>-setup.exe.blockmap `
  release/latest.yml
```

Both names are required, for different consumers:

- **`MagicBot-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`MagicBot-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/MagicBot-setup.exe` button a stable URL. This
  mirrors `MagicBot.dmg` sitting beside `MagicBot-<version>.dmg`.
