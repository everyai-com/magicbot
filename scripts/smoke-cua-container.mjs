...
const runtime = process.env.MB_CUA_RUNTIME || "docker";
const container = process.env.MB_CUA_CONTAINER;
const socket = process.env.MB_CUA_SOCKET || "/run/user/1000/magicbots-cua.sock";
if (!container || !/^[a-zA-Z0-9_.-]+$/.test(container)) {
  throw new Error("set MB_CUA_CONTAINER to the explicitly created smoke container name");
}
...
const child = spawn(
  ...
  [
    ...
    container,
    "/usr/local/libexec/magicbots/cua-driver",
    "mcp",
    "--socket",
    socket,
  ],
...
try {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "magicbots-container-smoke", version: "1" },
  });
