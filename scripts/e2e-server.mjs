// End-to-end check of a running MagicBots harness server — the exact
...
// codex turn, and — with --with-box + MB_E2E_BOX_TOKEN — box provisioning,
...
const PORT = Number(opt("--port", process.env.MB_PORT ?? 8799));
const BASE = `http://127.0.0.1:${PORT}`;
const WITH_BOX = flag("--with-box");
const KEEP_BOTS = flag("--keep-bots");
const BOX_TOKEN = process.env.MB_E2E_BOX_TOKEN ?? "";

const tag = Date.now().toString(36).slice(-6);
const marker = (s) => `mb-e2e-${s}-${tag}`;
...
async function main() {
  ...
  try {
    ...
    if (byKind.claudeAgent?.snapshot.state === "available") {
      ...
      await send(bot.id, `Run exactly one shell command: curl -sS https://example.com -o mb_${tag}_allow.html — then tell me the first word of the downloaded file.`);
      ...
      await send(denyBot.id, `Run the shell command "curl -sS https://example.org -o mb_${tag}_deny.html" — nothing else.`);
    ...
    // ── box: cloud computer ──
    if (WITH_BOX) {
      if (!BOX_TOKEN) fail("--with-box needs MB_E2E_BOX_TOKEN");
