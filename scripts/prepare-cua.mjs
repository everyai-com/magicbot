...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "magicbots", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "MagicBots-packager" } });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "magicbots-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __magicbotsCreateRequire } from "node:module"; const require = __magicbotsCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
...
await writeFile(
  ...
  bundledSource.replace(
    resolverPattern,
    `${resolvers[0]}\n      if (process.env.MAGICBOTS_CUA_SDK_LIBRARY) return resolveOverride(opts.crateName, process.env.MAGICBOTS_CUA_SDK_LIBRARY);`,
  ),
);
