...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "magicbot", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "MagicBot-packager" } });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "magicbot-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __magicbotCreateRequire } from "node:module"; const require = __magicbotCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
...
await writeFile(
  ...
  bundledSource.replace(
    resolverPattern,
    `${resolvers[0]}\n      if (process.env.MAGICBOT_CUA_SDK_LIBRARY) return resolveOverride(opts.crateName, process.env.MAGICBOT_CUA_SDK_LIBRARY);`,
  ),
);
