...
describe("resolveCuaMacArches", () => {
  ...
  it("rejects an empty override even when PARTIAL=1", () => {
    expect(() => resolveCuaMacArches({ MAGICBOT_CUA_ARCHES: "", MAGICBOT_CUA_ARCHES_PARTIAL: "1" })).toThrow(
      /MAGICBOT_CUA_ARCHES is empty/,
    );
    expect(() => resolveCuaMacArches({ MAGICBOT_CUA_ARCHES: " , ", MAGICBOT_CUA_ARCHES_PARTIAL: "1" })).toThrow(
      /MAGICBOT_CUA_ARCHES is empty/,
    );
  ...
  it("rejects a one-arch override unless PARTIAL=1", () => {
    expect(() => resolveCuaMacArches({ MAGICBOT_CUA_ARCHES: "arm64" })).toThrow(/omits x64/);
    expect(resolveCuaMacArches({ MAGICBOT_CUA_ARCHES: "arm64", MAGICBOT_CUA_ARCHES_PARTIAL: "1" })).toEqual([
      "arm64",
    ]);
  });
});
