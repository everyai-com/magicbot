...
describe("resolveCuaMacArches", () => {
  ...
  it("rejects an empty override even when PARTIAL=1", () => {
    expect(() => resolveCuaMacArches({ MAGICBOTS_CUA_ARCHES: "", MAGICBOTS_CUA_ARCHES_PARTIAL: "1" })).toThrow(
      /MAGICBOTS_CUA_ARCHES is empty/,
    );
    expect(() => resolveCuaMacArches({ MAGICBOTS_CUA_ARCHES: " , ", MAGICBOTS_CUA_ARCHES_PARTIAL: "1" })).toThrow(
      /MAGICBOTS_CUA_ARCHES is empty/,
    );
  ...
  it("rejects a one-arch override unless PARTIAL=1", () => {
    expect(() => resolveCuaMacArches({ MAGICBOTS_CUA_ARCHES: "arm64" })).toThrow(/omits x64/);
    expect(resolveCuaMacArches({ MAGICBOTS_CUA_ARCHES: "arm64", MAGICBOTS_CUA_ARCHES_PARTIAL: "1" })).toEqual([
      "arm64",
    ]);
  });
});
