...
const override = process.env.MB_ANDROID_PLATFORM_TOOLS_SOURCE;
const temporary = mkdtempSync(join(tmpdir(), "magicbots-android-tools-"));
const staged = join(temporary, platform);

try {
  if (override) {
    if (!existsSync(join(override, process.platform === "win32" ? "adb.exe" : "adb"))) {
      throw new Error(`MB_ANDROID_PLATFORM_TOOLS_SOURCE has no adb: ${override}`);
    }
