...
```sh
git clone https://github.com/everyai-com/magicbot
cd MagicBot
...
dns-sd -B _magicbot._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/MagicBot must be allowed, or the phone reaches
...
cd ios && xcodegen generate && open MagicBotCompanion.xcodeproj
```
...
```sh
xcodebuild -project MagicBotCompanion.xcodeproj \
  -scheme MagicBotCompanion \
  -sdk iphonesimulator \
...
```sh
xcodebuild -project MagicBotCompanion.xcodeproj -scheme MagicBotCompanion \
...
1. **Pair.** In MagicBot → Settings → Companion, choose **Set up a
   phone**. Scan the QR code with the phone's Camera, open MagicBotMobile,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → MagicBot → Local Network. If the
...
3. **In MagicBot → Settings → Companion:** with the toggle on, the panel now
...
- **Xcode console**, subsystem `com.magicbot.companion`: `opening stream`,
