...
dns-sd -B _magicbots._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/MagicBots must be allowed, or the phone reaches
...
cd ios && xcodegen generate && open MagicBotsCompanion.xcodeproj
```
...
```sh
xcodebuild -project MagicBotsCompanion.xcodeproj -scheme MagicBotsCompanion \
...
1. **Pair.** In MagicBots → Settings → Companion, choose **Set up a
   phone**. Scan the QR code with the phone's Camera, open MagicBotsMobile,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → MagicBots → Local Network. If the
...
3. **In MagicBots → Settings → Companion:** with the toggle on, the panel now
...
- **Xcode console**, subsystem `com.magicbots.companion`: `opening stream`,
