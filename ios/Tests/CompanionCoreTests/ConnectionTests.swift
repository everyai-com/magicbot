...
final class ConnectionTests: XCTestCase {
    ...
    func testParsesADesktopPairingInvite() throws {
        let token = "mb_pair_" + String(repeating: "a", count: 43)
        let url = try XCTUnwrap(URL(string: "magicbots://pair?address=macbook.tail1234.ts.net%3A8810&token=\(token)&code=004209&name=Milind%27s%20Mac"))
    ...
    func testParsesAnOlderCodeOnlyPairingInvite() throws {
        let url = try XCTUnwrap(URL(string: "magicbots://pair?address=mac.local&code=004209"))
        XCTAssertEqual(PairingInvite.parse(url)?.credential, "004209")
    }

    func testCarriesTheFallbackHostsFromTheInvite() throws {
        let url = try XCTUnwrap(URL(string:
            "magicbots://pair?address=macbook.tail1234.ts.net%3A8810&code=004209" +
            "&hosts=macbook.tail1234.ts.net,192.168.1.42,magicbots-aa.local"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
        XCTAssertEqual(invite.connection.hosts, ["macbook.tail1234.ts.net", "192.168.1.42", "magicbots-aa.local"])
    }
    ...
    func testDropsUnusableFallbackHostsWithoutRefusingTheInvite() throws {
        ...
        let url = try XCTUnwrap(URL(string:
            "magicbots://pair?address=mac.local&code=004209&hosts=%20192.168.1.42%20,,bad%2Fslash,has%20space"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
        XCTAssertEqual(invite.connection.hosts, ["192.168.1.42"])

        // and an invite with no usable candidate keeps the single address
        let empty = try XCTUnwrap(URL(string: "magicbots://pair?address=mac.local&code=004209&hosts=bad%2Fslash"))
    ...
    func testAPairResponseWithAndWithoutHostsDecodes() throws {
        let older = Data(#"{"token":"mb_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac"}"#.utf8)
        XCTAssertNil(try JSONDecoder().decode(PairResponse.self, from: older).hosts)

        let newer = Data(#"{"token":"mb_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac","hosts":["a.ts.net","192.168.1.42"]}"#.utf8)
    ...
    func testRejectsAnUntrustedOrMalformedPairingInvite() throws {
        ...
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "magicbots://pair?address=mac.local&code=12345"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "magicbots://pair?address=mac.local&token=weak"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "magicbots://pair?address=mac.local&token=weak&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "magicbots://pair?address=host%2Fpath&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "magicbots://pair?address=one.local&address=two.local&code=123456"))))
    }
