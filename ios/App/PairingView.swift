...
struct PairingView: View {
            ...
            .fullScreenCover(isPresented: $showingScanner) {
                PairingScannerSheet { payload in
                    guard let url = URL(string: payload), let invite = PairingInvite.parse(url) else {
                        return "That isn't a MagicBots pairing QR code."
                    }
                    accept(invite)
                    return nil
                }
    ...
    private var qrActionSection: some View {
        VStack(spacing: 8) {
            ...
            Text("In MagicBots, open Settings → Companion → Set up a phone to view your QR code.")
                .font(.caption2)
    ...
    private func confirmationView(for connection: Connection) -> some View {
        VStack(spacing: 16) {
            ...
            if let credential = scannedCredential {
                Text("Confirm this computer to establish an authenticated companion connection. Use a trusted Wi-Fi network or a tailnet; MagicBots does not encrypt local Wi-Fi traffic.")
                    .font(.caption)
                    .foregroundColor(isDark ? Color(hex: "#94A3B8") : Color(hex: "#64748B"))
                    .multilineTextAlignment(.center)
                    .padding(.horizontal, 16)
    ...
    private var discoveryStatus: String {
        ...
        if discovery.found.isEmpty {
            return discovery.browsing ? "Searching for MagicBots hosts…" : "Starting local discovery…"
        }
