...
struct UpdatesSheet: View {
    ...
    private func section(_ title: String, tint: Color?, kind: ChatUpdate.Kind) -> some View {
        ...
        if !items.isEmpty {
            let color = kind == .needsYou ? MascotPalette.color(items[0].chat.color) : Color.secondary
    ...
    var body: some View {
        Button(action: open) {
            HStack(alignment: .top, spacing: 12) {
                ChatAvatarView(chat: update.chat, size: 40, state: MascotState.forChat(update.chat, in: session.state))
                                        ...
                                        .background(
                                            Capsule().fill(
                                                CardStyle.isRefusal(option)
                                                    ...
                                                    : MascotPalette.color(update.chat.color)
                                            )
                                        )
                                }
                                .buttonStyle(.plain)
                ...
                case .toReview:
                    HStack(spacing: 6) {
                        Circle().fill(MascotPalette.color(update.chat.color)).frame(width: 10, height: 10)
