...
describe("the stored choice", () => {
  ...
  it("holds an opt-out for the session even when the write is rejected", async () => {
    ...
    fresh.initAnalytics();
    expect(store.get("mb-installed")).toBeUndefined();
  });
...
describe("initAnalytics while opted out", () => {
  it("returns before touching the client or the install marker", async () => {
    ...
    vi.resetModules();
    store.set("mb-analytics-opt-out", "1"); // as a previous session left it
    const fresh = await import("./analytics");
  ...
  it("honours a pre-rename opt-out until the user opts back in", async () => {
    vi.resetModules();
    store.set("omb-analytics-opt-out", "1"); // as a pre-rename session left it
    const fresh = await import("./analytics");
    ...
    expect(fresh.analyticsEnabled()).toBe(true);
