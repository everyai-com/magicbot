...
export function ManageMembersPanel({
  ...
  const save = () => {
    ...
    if (rosterChanged) {
      setSaveError("This team's members changed while the panel was open. Close it and try again.");
      return;
    }
    if (changed) {
  ...
  return (
    ...
    >
      <div
        ...
        className="w-[min(340px,calc(100vw-2rem))] rounded-2xl border border-hairline/50 bg-card p-4 shadow-2xl"
      >
        ...
        <BotPickerList bots={bots} picked={picked} onToggle={toggle} emptyHint="Create a bot first — teams are made of bots." />
        {!memberIds.length && <div className="mt-2 text-[12px] text-ink-secondary">A team needs at least one bot.</div>}
        {saveError && (
