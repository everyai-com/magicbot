...
export function CampaignOutcomes({ rows, onExport, agentId, campaignName, exportTarget, resultValues }: { resultValues?: string[]; exportTarget?: HTMLDivElement | null; rows: Row[]; agentId?: string; campaignName: string; onExport: (rows: Row[]) => void }) {
  ...
  return <div className="min-w-0 space-y-3">
    {errors.length > 0 && <div role="alert" className="rounded-lg border border-danger/30 p-3 text-sm text-danger"><p className="font-medium">Delivery errors</p>{errors.map((error) => <p key={error} className="mt-2 whitespace-pre-wrap break-words">{error}</p>)}</div>}
