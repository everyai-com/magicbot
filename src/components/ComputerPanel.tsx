...
export function ComputerPanel({ bot }: { bot: Bot }) {
  ...
  const openDesktop = async () => {
    ...
    try {
      ...
      if (window.ogb?.desktopViewer) {
        ...
        if (!opened) throw new Error("MagicBot could not open the live desktop");
      } else if (fallbackTab) {
        ...
        if (!opened) throw new Error("MagicBot could not open the live desktop link");
  ...
  const replaceVpsComputer = async () => {
    if (!window.confirm(`Replace ${bot.name}'s VPS computer with the version required by this MagicBot update? Files stored only inside the disposable container will be deleted.`)) return;
  ...
  const openVmSettings = () => {
    window.sessionStorage.setItem("magicbot.settings.section", "computer");
  ...
  const emptyState = {
    ...
    "vps-incompatible": "This VPS computer belongs to an earlier MagicBot version",
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-5">
        ...
        {phase === "vm" && vmViewerUrl && control.held && (
          <button
            ...
            title="Open the Local VM's live desktop inside MagicBot"
          >
