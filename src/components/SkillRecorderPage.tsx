...
export function SkillRecorderPage() {
  ...
  const start = async () => {
    ...
    if (!bridge || !window.ogb?.beginScreenPreviewIntent || !navigator.mediaDevices?.getDisplayMedia) {
      setError("Skill recording requires the MagicTeams desktop app on macOS.");
      return;
    }
            ...
            <div className="mx-auto max-w-2xl">
              <div className="rounded-3xl border border-hairline bg-panel p-8 shadow-sm">
                ...
                <p className="mt-2 max-w-xl text-[14px] leading-6 text-ink-secondary">
                  Speak naturally while you work. MagicTeams lines up your clicks, app changes, screenshots, and narration, then turns the reviewed demonstration into a reusable local skill.
                </p>
