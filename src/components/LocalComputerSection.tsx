...
export function LocalComputerSection() {
  ...
  return (
    <>
      ...
      <Card title="Setup" subtitle="Once a container runtime is open, MagicBot prepares Cua and the VM for you.">
        <div className="flex flex-col gap-4">
          ...
          >
            {perBot ? (
              <div className="text-[13px] leading-relaxed text-ink-secondary">
                {perBotRuntimeUnsupported
                  ? "Apple container requires an explicit host port, so MagicBot will not guess or expose one. Install or start Docker or Podman for safe per-bot dynamic loopback ports."
                  : <>
                      Choose <b className="text-ink">Local VM</b> for a bot, open that bot's Computer panel, then create its desktop there. MagicBot assigns a private workspace and an available loopback viewer port automatically.
                    </>}
              </div>
            ) : needsRecreate ? (
              <>
      ...
      {unavailable && (
        <Card>
          <div className="flex gap-2 text-[13px] text-ink-secondary">
            ...
            <span>MagicBot could not inspect the container runtime. Re-check, or review the app logs.</span>
          </div>
        </Card>
      )}
