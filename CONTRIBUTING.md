...
- stable `MagicBots-amd64.deb` and `MagicBots.AppImage` copies used by the latest-download links;
...
[`magicbot-releases`](https://github.com/everyai-com/magicbot-releases) repository. Then verify the checksum
...
Data lives in `~/.magicbots/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.magicbots`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.magicbots/config.json` via `PUT /api/config` and the API
