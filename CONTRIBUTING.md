...
- stable `MagicBot-amd64.deb` and `MagicBot.AppImage` copies used by the latest-download links;
...
[`magicbot`](https://github.com/everyai-com/magicbot) repository. Then verify the checksum
...
Data lives in `~/.magicbot/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.magicbot`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.magicbot/config.json` via `PUT /api/config` and the API
