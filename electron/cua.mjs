...
//    EmbeddedCuaDriverHost so TCC grants attribute to MagicBots and the
//    driver inherits them. One prompt, named MagicBots, out of the box.
...
const HOST_BUNDLE_ID = "com.magicbots.app";
...
async function loadEmbeddedSdk() {
  ...
  process.env.MAGICBOTS_CUA_SDK_LIBRARY = path.join(
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed MagicBots.
...
async function startEmbedded(binary) {
  ...
  // MagicBots rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart MagicBots`);
  }
...
export async function startCua() {
  ...
  const wantEmbedded =
    app.isPackaged || process.env.MAGICBOTS_CUA_EMBEDDED === "1";
  let nextConnection;
