...
const BUNDLE = app.isPackaged
  ? path.join(process.resourcesPath, "MagicBots Recorder.app")
  : recorderHelperBundle;
...
export function startRecorder(win) {
  ...
  ensureBuilt();
  const sessionDir = mkdtempSync(path.join(app.getPath("temp"), "magicbots-recorder-"));
...
export function saveSkillRecording(payload, options = {}) {
  ...
  const dataRoot = options.dataRoot ?? process.env.MB_DATA_DIR ?? path.join(os.homedir(), ".magicbots");
