...
const BUNDLE = app.isPackaged
  ? path.join(process.resourcesPath, "MagicBot Recorder.app")
  : recorderHelperBundle;
...
export function startRecorder(win) {
  ...
  ensureBuilt();
  const sessionDir = mkdtempSync(path.join(app.getPath("temp"), "magicbot-recorder-"));
...
export function saveSkillRecording(payload, options = {}) {
  ...
  const dataRoot = options.dataRoot ?? process.env.OMB_DATA_DIR ?? path.join(os.homedir(), ".magicbot");
