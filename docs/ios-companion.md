...
The iOS app is a thin, native client for the MagicBot instance running on
...
The sidecar keeps its device registry in `~/.magicbot/devices.json`. That is
...
The sidecar advertises `_magicbot._tcp` over Bonjour. The app browses with
...
Tailscale is optional. There is no MagicBot-operated relay or cloud copy of
...
one-time exchange for a securely stored long-lived credential. An MagicBot
