# Computer use & browser use in MagicBots

Decision doc, 2026-08-12. How bots in MagicBots get local computer use and
...
supply-chain contract tracked in [#113](https://github.com/everyai-com/magicbot/issues/113): pinned archive
...
receives the validated MCP proxy contract. Xorg is tracked in [#79](https://github.com/everyai-com/magicbot/issues/79);
GNOME/Wayland additionally requires WinRects v8 plus the exact Cua health-report contract tracked in
[#109](https://github.com/everyai-com/magicbot/issues/109).
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is MagicBots's, users see ONE
   prompt named MagicBots, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `MagicBots.app/Contents/Resources/cua-driver`,
...
{ "mcpServers": { "computer": {
    ...
    "env": { "CUA_DRIVER_EMBEDDED": "1", "CUA_DRIVER_HOST_BUNDLE_ID": "com.magicbots.app" }
} } }
```
