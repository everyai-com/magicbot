...
MagicBots can turn a Linux server you already own into a bot's computer. The agent process stays on your
...
only surface exposed: MagicBots never opens a port on the VPS, never stores a key or password, and never
...
MagicBots connects only through a named alias in your `~/.ssh/config` — you type the alias into
...
**Host key first, by hand.** Connect once manually before pointing MagicBots at the alias:
...
- **No public ports.** The managed container is created with no published ports, and MagicBots refuses to
  use a container that publishes any — the check runs before every attach, not just at creation.
- **Firewall the VPS to SSH only**, ideally from your IP. Nothing MagicBots does needs any other inbound
  port open, so anything else open is pure attack surface.
- **Nothing sensitive is stored.** The only thing MagicBots persists is the alias name itself
  (`~/.magicbots/config.json`); keys, passphrases, and agent state stay with SSH. The alias is also kept
...
Each bot owns one container on the VPS, named `magicbots-vps-<bot>-<hash>` — stable across restarts and
...
- **Remove** is yours, done by hand when a bot no longer needs the server:
  `docker -H ssh://my-vps rm -f <container>`. MagicBots never deletes a container on its own.
