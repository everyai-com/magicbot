# MagicBots harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what MagicBots lacked by
...
**The reframe that reorders everything.** MagicBots is itself a harness. The
...
**On-disk forward compatibility.** `~/.magicbots` is shared across app versions.
