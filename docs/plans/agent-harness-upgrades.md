# MagicBot harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what MagicBot lacked by
...
**The reframe that reorders everything.** MagicBot is itself a harness. The
...
**On-disk forward compatibility.** `~/.magicbot` is shared across app versions.
