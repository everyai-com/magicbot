# MagicBot Harness Upgrades v2 Implementation Plan
...
**Architecture:** MagicBot is itself a harness; `server/store.ts` is the
...
- `~/.magicbot` is shared across app versions — every new field or message kind must be ignorable by an older build.
...
messages **per thread** to `join(home, ".magicbot", "native",
