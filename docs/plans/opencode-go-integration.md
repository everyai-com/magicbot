...
Add OpenCode Go as a first-class MagicBots engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse MagicBots's ACP runtime rather than call
...
provide inference; MagicBots does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. MagicBots should
...
the process lifecycle MagicBots uses for Grok and Gemini.
...
- load that session on the next MagicBots turn;
...
- concurrent MagicBots threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; MagicBots never selects it as a runnable default otherwise.
- The setup flow works without restarting MagicBots and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so MagicBots does not double-count it?
