...
MagicBots has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded MagicBots server on GNOME Xorg and GNOME Wayland.
...
- [Debian package (`MagicBots-amd64.deb`)](https://github.com/everyai-com/magicbot-releases/releases/latest/download/MagicBots-amd64.deb) — recommended; APT installs its desktop dependencies.
- [Portable AppImage (`MagicBots.AppImage`)](https://github.com/everyai-com/magicbot-releases/releases/latest/download/MagicBots.AppImage) — does not install system files.
- [SHA-256 checksums](https://github.com/everyai-com/magicbot-releases/releases/latest/download/SHA256SUMS-ubuntu-x64.txt)

Versioned packages and previous releases remain available on the
[releases page](https://github.com/everyai-com/magicbot-releases/releases).
...
- `release/MagicBots-<version>-amd64.deb`
- `release/MagicBots-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./MagicBots-amd64.deb
```

Then open **MagicBots** from the GNOME application launcher. To remove it:

```sh
sudo apt remove magicbots
```

The portable AppImage does not install system files:

```sh
chmod +x release/MagicBots-*-x86_64.AppImage
./release/MagicBots-*-x86_64.AppImage
```

For a downloaded release AppImage, use `MagicBots.AppImage` in place of the versioned path above.

Application data remains local in `~/.magicbots`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/magicbots` unless the environment overrides it).
...
./release/linux-unpacked/magicbots
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. MagicBots
...
```sh
MB_EXTRA_PATH=/your/custom/bin ./release/MagicBots-*-x86_64.AppImage
```

Restart MagicBots after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. MagicBots detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. MagicBots does not capture screen audio, remember the selected monitor after restart, or
offer an **Open Settings** action on Linux.

Local computer control is a separate opt-in. On Wayland, MagicBots recognizes only GNOME/Mutter and requires
...
You do not need to install Cua separately for GNOME/Xorg. MagicBots starts its own private daemon only after
...
GNOME/Wayland still needs the privileged WinRects v8 Shell helper. MagicBots does not install or enable a Shell
...
The output must include `Version: 8`, `Enabled: Yes`, and `State: ACTIVE`. MagicBots never installs or enables
...
MagicBots starts one private embedded daemon with a private socket for its own app generation. It never touches
...
MagicBots requires its own **Allow** or **Deny** decision before every local action. Bot Auto mode, persistent
**Always allow** grants, and cloud-computer approvals cannot authorize the local desktop in this beta.

Cua Driver has content-free telemetry and an update check enabled by default. MagicBots disables both for every
Cua process it owns and does not change any separately installed Cua preferences. Driver updates arrive only with an
MagicBots application release; rolling back the app rolls back the paired driver. Review the upstream behavior in
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart MagicBots. If it lives outside the
common directories above, use `MB_EXTRA_PATH` while testing and report the install location so it can be
...
driver=/opt/MagicBots/resources/cua-linux-x64/cua-driver
...
CUA_DRIVER_RS_ENABLE_WAYLAND=1 \
  /opt/MagicBots/resources/cua-linux-x64/cua-driver doctor --json
```

If the helper is installed but not `ACTIVE`, sign out and back in once. If the app reports a portal error, confirm
that `xdg-desktop-portal` and `xdg-desktop-portal-gnome` are running in the user session. MagicBots's readiness
...
is unexpected, close MagicBots and launch it with an absolute `CUA_DRIVER_PATH`. An invalid explicit override
...
```sh
chmod +x MagicBots-*-x86_64.AppImage
file MagicBots-*-x86_64.AppImage
```
