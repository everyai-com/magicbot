...
Cursor is an optional MagicBot engine. MagicBot runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; MagicBot already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, MagicBot pins the same id over
...
switch). MagicBot still answers ACP `session/request_permission` itself:
