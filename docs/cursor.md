...
Cursor is an optional MagicBots engine. MagicBots runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; MagicBots already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, MagicBots pins the same id over
...
switch). MagicBots still answers ACP `session/request_permission` itself:
