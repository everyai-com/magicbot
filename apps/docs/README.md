...
```bash
pnpm docs:build
pnpm --filter @magicbot/docs types:check
pnpm --filter @magicbot/docs lint
```
...
Create a second Vercel project beside the existing `magicbot.com` project:
...
5. Add `docs.magicbot.com` under **Settings → Domains**.

Vercel will build the static `out` directory, publish every push to `main`, and create preview URLs for documentation pull requests. Keep `magicbot.com` on the existing marketing project and add a Docs link there after the new domain is live.
