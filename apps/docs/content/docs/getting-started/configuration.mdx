...
Most users configure MagicBot from the app. The packaged desktop validates credentials before saving them and stores supported secrets with the operating system's secure storage.

## Engine discovery

MagicBot checks the inherited process path, common install locations, and the login shell. If an engine is installed somewhere unusual, open **Settings → Engines** and choose its executable explicitly.
...
- launch MagicBot from a Windows shortcut or Linux application menu with a different environment from your terminal.
