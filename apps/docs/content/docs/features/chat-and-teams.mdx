...
Editing an earlier message creates a new conversation path. MagicBots avoids resuming stale provider sessions after a rewind, so the next turn follows the visible branch rather than hidden abandoned state.
