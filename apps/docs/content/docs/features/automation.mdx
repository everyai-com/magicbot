...
<ProductScreenshot name="docs-automations" alt="An empty MagicBot automations calendar ready for its first schedule" caption="Review work across a day, three-day, or week view and filter by bot." />

Run receipts distinguish queued, active, waiting, completed, missed, failed, and cancelled work. MagicBot must be running when a local routine becomes due.
...
The receiver exposes only health and secret hook routes. It does not expose the broader MagicBot API.
...
<Callout type="warn" title="MagicBot must remain running">
