...
<ProductScreenshot name="docs-automations" alt="An empty MagicBots automations calendar ready for its first schedule" caption="Review work across a day, three-day, or week view and filter by bot." />

Run receipts distinguish queued, active, waiting, completed, missed, failed, and cancelled work. MagicBots must be running when a local routine becomes due.
...
The receiver exposes only health and secret hook routes. It does not expose the broader MagicBots API.
...
<Callout type="warn" title="MagicBots must remain running">
