...
MagicBot does not proxy every message through its own model account. It starts supported agent CLIs on your computer and normalizes their output through the local harness.

<ProductScreenshot name="docs-engine-detection" alt="MagicBot checking which agent engines are installed" caption="The official app checks installation and sign-in state without asking users to find executable paths first." />
...
MagicBot checks common installation directories and the login shell in the background. Restart the app after installing or authenticating a CLI.
...
<ProductScreenshot name="docs-model-picker" alt="The MagicBot model picker with several cloud and local engines" caption="Every bot can use a different engine and model." />

<Callout type="info" title="Subscriptions stay with the provider">
  Your existing CLI login or provider API key determines access and billing. MagicBot does not turn one subscription into access to another provider.
</Callout>
