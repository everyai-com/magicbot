...
description: Know what MagicBots stores locally and what to preserve.
icon: DatabaseBackup
---

MagicBots keeps its application state under `~/.magicbots`, including configuration, transcripts, message attachments, device pairing state, and the SQLite message store.

## Back up

Close MagicBots before taking a filesystem-level backup so SQLite and configuration files are captured consistently. Preserve the entire `.magicbots` directory rather than selecting individual database files.
