...
description: Know what MagicBot stores locally and what to preserve.
icon: DatabaseBackup
---

MagicBot keeps its application state under `~/.magicbot`, including configuration, transcripts, message attachments, device pairing state, and the SQLite message store.

## Back up

Close MagicBot before taking a filesystem-level backup so SQLite and configuration files are captured consistently. Preserve the entire `.magicbot` directory rather than selecting individual database files.
