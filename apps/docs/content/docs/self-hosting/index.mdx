---
title: Self-hosting MagicBot
description: Run the harness and desktop app on infrastructure you control.
icon: Server
---

MagicBot is local-first by default: the harness listens on loopback, agent CLIs run on your machine, and application state lives under `~/.magicbot`.

## Typical self-hosted setup

1. Run MagicBot on your personal macOS, Windows, or Ubuntu computer.
...
Self-hosting MagicBot does not self-host every optional provider:
