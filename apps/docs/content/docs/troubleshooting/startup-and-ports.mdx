...
1. Quit every MagicBot window.
2. Confirm no old MagicBot process remains.
3. Start the app once and wait for the local harness.
4. If it still fails, inspect the desktop log.

On macOS, packaged logs are under `~/Library/Logs/MagicBot`. On Windows they are under the MagicBot log directory in `%APPDATA%`.
...
Developers can set `OMB_DATA_DIR` to run against a clea MagicBot data directory. This is useful for reproduction and screenshots without touching a real workspace.
