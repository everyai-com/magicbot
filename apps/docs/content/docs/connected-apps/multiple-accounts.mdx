...
MagicBots supports several labeled accounts for the same toolkit. This is useful for two Gmail inboxes, several Slack workspaces, or separate personal and work services.
...
MagicBots caps each toolkit at five usable accounts. If a provider or Composio project policy prevents another authorization, use a separate MagicBots installation or configuration rather than reauthorizing over a working account.
