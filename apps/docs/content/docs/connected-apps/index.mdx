...
Open **Connected apps** from the sidebar, choose a service, and complete its authorization in your normal browser. MagicBot turns the resulting connection into tools that supported agent engines can use.

<ProductScreenshot name="docs-connected-apps" alt="The clea MagicBot connected-app marketplace" caption="Search the marketplace or review every account from the Connected tab." />

## Official packaged app

The signed desktop app registers its own installation with MagicBot's managed connected-apps service. You do **not** need to find or paste a Composio key.
...
Connect more than one account for the same toolkit and give each a clear alias such as `work`, `personal`, or `client-acme`. When several accounts could perform an action, MagicBot requires explicit selection instead of silently replacing the first connection.
