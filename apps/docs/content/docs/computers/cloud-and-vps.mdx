...
Box is a third-party paid service after its trial. MagicBot stores the configured credential locally and does not expose it to the renderer.

## Bring your own VPS

MagicBot can control Docker on an x86_64 Linux server through an SSH config alias. The agent process stays on your local machine; only Docker commands and the managed desktop container run on the VPS.
