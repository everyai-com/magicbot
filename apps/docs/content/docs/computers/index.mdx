...
A bot can work without a computer, use the computer running MagicBot, or work inside an isolated desktop.

<ProductScreenshot name="docs-computer-panel" alt="MagicBot computer panel offering cloud, local VM, host computer, and off modes" caption="Computer placement is explicit per bot and can be changed without changing its model." />
...
The Computer panel can show live frames while an agent works. Supported cloud desktops also provide an interactive viewer. MagicBot opens that viewer in the app when possible, with a browser fallback for external join flows.
