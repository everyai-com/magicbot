...
A bot can work without a computer, use the computer running MagicBots, or work inside an isolated desktop.

<ProductScreenshot name="docs-computer-panel" alt="MagicBots computer panel offering cloud, local VM, host computer, and off modes" caption="Computer placement is explicit per bot and can be changed without changing its model." />
...
The Computer panel can show live frames while an agent works. Supported cloud desktops also provide an interactive viewer. MagicBots opens that viewer in the app when possible, with a browser fallback for external join flows.
