...
MagicBot's harness listens on `127.0.0.1` and nothing else, which is the
...
| `OMB_COMPANION_DIR` | `~/.magicbot-companion` | paired devices live here |
...
back to `MagicBot` when the harness is not up or has no profile. Read once
