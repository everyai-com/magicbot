...
MagicBots's harness listens on `127.0.0.1` and nothing else, which is the
...
| Environment | Default | |
|---|---|---|
| `MB_PORT` | `8799` | where the harness is |
| `MB_WEBHOOK_PORT` | `MB_PORT` + 1 | the harness's webhook receiver — refused, not used |
| `MB_COMPANION_PORT` | `8810` | where devices connect |
| `MB_CONTROL_PORT` | `8811` | the pairing page, loopback only |
| `MB_COMPANION_DIR` | `~/.magicbots-companion` | paired devices live here |
| `MB_COMPANION_NAME` | your name, from the harness | what the phone calls this computer |

`MB_COMPANION_NAME` overrides a name the sidecar otherwise asks the harness
for at startup — the profile from onboarding, as *"Ada's computer"*. It falls
back to `MagicBots` when the harness is not up or has no profile. Read once
...
it (`MB_WEBHOOK_PORT`). The companion refuses to start on either and says
