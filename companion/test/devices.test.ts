...
describe("DeviceRegistry", () => {
  ...
  it("issues a token that authenticates, and never stores it", () => {
    ...
    expect(token.startsWith("mb_")).toBe(true);
  ...
  it("refuses unknown, empty, and near-miss tokens", () => {
    ...
    expect(registry.authenticate("mb_nope")).toBeNull();
  ...
  it("uses a high-entropy QR credential and burns the manual fallback with it", () => {
    ...
    expect(token).toMatch(/^mb_pair_[A-Za-z0-9_-]{43}$/);
...
describe("bearerToken", () => {
  it("reads only a well-formed Bearer header", () => {
    expect(bearerToken("Bearer mb_abc")).toBe("mb_abc");
    expect(bearerToken("  Bearer mb_abc  ")).toBe("mb_abc");
    expect(bearerToken("mb_abc")).toBeUndefined();
    expect(bearerToken("Basic mb_abc")).toBeUndefined();
  ...
  it("matches the scheme however it is cased", () => {
    ...
    expect(bearerToken("Bearer   ")).toBeUndefined();
    expect(bearerToken("Bearermb_abc")).toBeUndefined();
  });
});
