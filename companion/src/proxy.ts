  ...
  return function handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (isCloudDesktopJoin(method, path) && !device?.cloudDesktopAccess) {
      return sendJson(res, 403, {
        error: "cloud desktop access is off for this phone — enable it in MagicBot → Settings → Companion",
      });
    }
      ...
      (harness) => {
        ...
        harness.on("data", (chunk: Buffer) => {
          ...
          if (size > MAX_JSON_BODY_BYTES) {
            ...
            else sendJson(res, 502, { error: "the response from MagicBot was too large" });
            return;
          }
          chunks.push(chunk);
        });
    ...
    upstream.on("error", () => {
      ...
      sendJson(
        ...
        timedOut ? 504 : 502,
        timedOut
          ? { error: "MagicBot did not respond" }
          : { error: "MagicBot is not running on this computer" },
      );
    });
    req.pipe(upstream);
  };
